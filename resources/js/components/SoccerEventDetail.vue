<template>
    <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-all" role="tabpanel" aria-labelledby="nav-all-tab">
            <div class="result-table-sec" v-if="typeof odds.fulltime_result != 'undefined'">
                <h4>Fulltime Result</h4>
                <div class="col-6 col-md-4 player-cat" v-for="row in odds.fulltime_result">
                    <span class="part-nme">{{ row.title }}</span>
                    <span class="partnumb"> {{ parseFloat(row.odds).toFixed(2) }} </span>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="result-table-sec" v-if="typeof odds.double_chance != 'undefined'">
                <h4>Double Chance</h4>
                 <div class="col-6 col-md-4 player-cat" v-for="row in odds.double_chance">
                    <span class="part-nme">{{ row.title }}</span>
                    <span class="partnumb"> {{ parseFloat(row.odds).toFixed(2) }} </span>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="result-table-sec" v-if="typeof odds.half_time_result != 'undefined'">
                <h4>Half Time Result</h4>
                 <div class="col-6 col-md-4 player-cat" v-for="row in odds.half_time_result">
                    <span class="part-nme">{{ row.title }}</span>
                    <span class="partnumb"> {{ parseFloat(row.odds).toFixed(2) }} </span>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="result-table-sec" v-if="typeof odds.first_goal != 'undefined'">
                <h4>1st Goal</h4>
                 <div class="col-6 col-md-4 player-cat" v-for="row in odds.first_goal">
                    <span class="part-nme">{{ row.title }}</span>
                    <span class="partnumb"> {{ parseFloat(row.odds).toFixed(2) }} </span>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="result-table-sec borde-non" v-if="typeof odds.match_goals != 'undefined'">
                <h4>Match Goals</h4>
                ...
                <div class="col-6 col-md-4 player-cat parti-goal bord-rght bord-bottm">
                    <span class="part-nme">{{ odds.match_goals[0].handicap }}</span>
                </div>
                <div class="col-6 col-md-4 player-cat parti-goal bord-bottm">
                    <span class="partnumb">{{ parseFloat(odds.match_goals[0].odds).toFixed(2) }}</span>
                </div>
                <div class="col-6 col-md-4 player-cat parti-goal bord-bottm">
                    <span class="partnumb">{{ parseFloat(odds.match_goals[0].odds).toFixed(2) }}</span>
                </div>
            ...
            <!-- <div class="result-table-sec borde-non" v-if="typeof odds.goals.sp.alternative_total_goals != 'undefined'">
                <h4>Alternative Match Goals</h4>

                <div class="col-6 col-md-4 participant-border">&nbsp</div>
                <div class="col-6 col-md-4 participant-border">Over</div>
                <div class="col-6 col-md-4 participant-border">Under</div>
                <div v-for="(label, index) in alternative_total_goals.label">
                    <div class="col-6 col-md-4 player-cat parti-goal bord-rght bord-bottm"><span class="part-nme">{{ label }}</span></div>
                    <div class="col-6 col-md-4 player-cat parti-goal bord-bottm"><span class="partnumb">{{ alternative_total_goals.over[index] }}</span></div>
                    <div class="col-6 col-md-4 player-cat parti-goal bord-bottm"><span class="partnumb">{{ alternative_total_goals.under[index] }}</span></div>
                </div>
                <div class="clearfix"></div>
            </div> -->
...
export default Vue.extend({
    ...
    computed:{
        ...
        alternative_total_goals() {
            ...
            if(typeof this.odds.goals != 'undefined') {
                ...
                this.odds.goals.sp.alternative_total_goals.forEach((row) => {
    ...
    methods: {
        loadOdds() {
            let self = this;
            axios.get('/prematch/odds/'+this.event.id+'?sportID=1')
            .then(function(response){
                self.odds = response.data
            })
        }
    }
})
